...
/**
 * AddJourneyActivity
 * واجهة إضافة رحلة جديدة مع ميزة تحديد الموقع الجغرافي (GPS) واختيار الصور.
 */
public class AddJourneyActivity extends AppCompatActivity {

    // ================== عناصر الواجهة (المستخدمة فعلياً) ==================
    ...
    // Launchers لصلاحيات الوسائط (الصور)
    ...
    private void initViews() {
        // تم حذف العناوين (TextViews) والـ RadioButtons الفردية لأنها غير مستخدمة برمجياً
    ...
    private void initPermissionLaunchers() {
        ...
        // طلب صلاحيات الصور (حسب إصدار الأندرويد)
    ...
    private void validateFields() {
        ...
        // إنشاء كائن الرحلة وتعبئة البيانات
        ...
        // تحديد نوع الرحلة من الـ RadioGroup مباشرة
    ...
    private void saveMyJourney(MyJourney myJourney) {
        ...
        DatabaseReference journeysRef = database.child("Journeys");
        DatabaseReference newJourneyRef = journeysRef.push();
        
        myJourney.setKey(newJourneyRef.getKey());
        
        newJourneyRef.setValue(myJourney).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                Toast.makeText(AddJourneyActivity.this, "Journey saved successfully!", Toast.LENGTH_SHORT).show();
                finish(); // إغلاق الصفحة بعد الحفظ
            } else {
    ...
    public String convertImageToString(Uri uri) {
        try {
            ...
            // ضغط الصورة لتقليل حجمها قبل الرفع
